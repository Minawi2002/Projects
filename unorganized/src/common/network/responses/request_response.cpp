...
request_response *request_response::from_json(const rapidjson::Value& json) {
    if (json.HasMember("err") && json.HasMember("success")) {
        ...
        throw UnoException("Could not parse request_response from json. err or success is missing.");
    }
}


std::string request_response::get_error(){
    return _err;
}

std::string request_response::get_req_id(){
    return _req_id;
}
    
bool request_response::get_success(){
    return _success;
}
