...
class ImageCompressor:
    ...
    def get_codebook(self):
        ...
        print("compressed U shape:")
        print(self.compressed_U.shape)

        codebook = np.concatenate((self.compressed_U.T, self.mean), axis=0)
    ...
    def train(self, train_images):
        ...
        stacked_images = np.stack(train_images, axis=0)
        print("stacked_images shape:", stacked_images.shape)
        stacked_images = (stacked_images.astype(np.float64) / 255.0)

        #print(stacked_images.shape)
        stacked_images = stacked_images.reshape(N,-1)
        print("stacked_images shape:", stacked_images.shape)
        self.mean = np.mean(stacked_images, axis=0, keepdims=True)
        ...
        U, S, V = np.linalg.svd(demeaned_images.T, full_matrices=False)
    ...
    def compress(self, test_image):
        ...
        z = np.dot(U_transpose, test_image.T)
